export interface CappedHealingResult {
  requested: number;
  allowed: number;
  remaining: number;
}

export function weaponSwitchDuration(quickSlingStacks: number): number {
  const stacks = Math.max(0, Math.floor(quickSlingStacks));
  return Math.max(0.12, 0.32 / (1 + stacks * 0.3));
}

export function recoilMultiplier(counterweightStacks: number): number {
  const stacks = Math.max(0, Math.floor(counterweightStacks));
  return Math.max(0.28, 1 - stacks * 0.18);
}

export function lifeStealCap(lifeStealStacks: number): number {
  return Math.max(0, Math.floor(lifeStealStacks)) * 18;
}

export function cappedHealingRequest(
  requested: number,
  healedThisWave: number,
  cap: number,
): CappedHealingResult {
