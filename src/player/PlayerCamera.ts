...
import { clampLookDelta, damp, type LookInputSource } from './controlMath';
...
export class PlayerCamera {
  ...
  look(dx: number, dy: number, aiming: boolean, source: LookInputSource): void {
    const base =
      source === 'touch'
  ...
  update(
    ...
    const sway =
      Math.cos(this.bobTime * 0.5) * 0.018 * moving * this.settings.headBob * reduced;
    const shakeX =
      (Math.random() - 0.5) * this.shake * 0.025 * this.settings.cameraShake * reduced;
    const shakeY =
      (Math.random() - 0.5) * this.shake * 0.025 * this.settings.cameraShake * reduced;
    const renderedPitch = Math.max(
      -1.5,
      Math.min(1.5, this.pitch - this.recoil * 0.035 + shakeY),
    );
