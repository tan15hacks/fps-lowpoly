export class AudioManager {
    ...
    setSettings(settings) { this.settings = settings; if (this.master)
    ...
    startAmbient() {
        ...
        gain.gain.value = 0.018 * this.settings.musicVolume;
        osc.connect(gain).connect(this.master);
        osc.start();
        this.ambient = osc;
    }
    stopAmbient() { this.ambient?.stop(); this.ambient = undefined; }
    play(id) {
        if (!this.context || !this.master || this.settings.effectsVolume <= 0)
            return;
        ...
        osc.frequency.exponentialRampToValueAtTime(Math.max(30, frequency * 0.55), now + duration);
        gain.gain.setValueAtTime(volume * this.settings.effectsVolume, now);
