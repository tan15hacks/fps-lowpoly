...
export function defaultPermanent() {
...
export function createDefaultSave() {
    ...
    for (const achievement of ACHIEVEMENTS)
        achievements[achievement.id] = { id: achievement.id, progress: 0 };
    return {
...
export function migrateSave(raw) {
    ...
    for (const key of Object.keys(permanent))
        permanent[key] = Math.round(clamp(permanent[key], 0, 5));
    const achievements = { ...base.achievements, ...(input.achievements ?? {}) };
    const owned = Array.isArray(input.ownedCosmetics) ? input.ownedCosmetics.filter((item) => typeof item === 'string') : ['standard'];
    if (!owned.includes('standard'))
        owned.unshift('standard');
...
export function validateImportedSave(raw) {
    if (!raw || typeof raw !== 'object')
        return { valid: false, reason: 'Save file is not a JSON object.' };
    const candidate = raw;
    if (typeof candidate.schemaVersion !== 'number')
        return { valid: false, reason: 'Missing schema version.' };
    if (candidate.credits !== undefined && (typeof candidate.credits !== 'number' || candidate.credits < 0))
        return { valid: false, reason: 'Invalid credit balance.' };
...
export class SaveManager {
    ...
    replace(data) { this.data = migrateSave(data); this.saveNow(); }
    ...
    readFallback() {
        try {
            return JSON.parse(localStorage.getItem('polygon-outpost-save') ?? 'null');
        }
        catch {
            return null;
        }
