...
import { resolveBackAction, shouldPauseForBackground, SingleFlightGuard, } from './RunLifecycle';
export class Game {
    host;
    canvas;
    save = new SaveManager();
    bus = new EventBus();
    materials = new LowPolyMaterialFactory();
    effects = new ScreenEffects();
    damageNumbers = new DamageNumbers();
    tutorial = new TutorialOverlay();
    ads = new UnavailableAdService();
    sceneManager;
    map;
    input;
    mobile;
    ...
    startGuard = new SingleFlightGuard();
    constructor(host) {
        this.host = host;
    ...
    async initialize() {
        ...
        this.canvas.addEventListener('pointerdown', (event) => {
            ...
            if (!this.input.prefersTouchControls())
                this.input.requestPointerLock();
        });
        this.input.onPointerLockLost = () => {
            if (this.run?.state !== 'playing' || this.input.prefersTouchControls())
                return;
    ...
    handleBackButton() {
        ...
        if (action === 'keep-death-dialog' || action === 'consume')
            return true;
        if (action === 'pause') {
            this.pause();
            return true;
    ...
    pauseForBackground() {
        if (shouldPauseForBackground(this.run?.state))
            this.pause();
    }
    async startRun(difficulty) {
        if (!this.startGuard.tryEnter())
            return;
        try {
            ...
            weapons.setAimDirectionResolver((origin, direction, range, maximumAngle) => {
                ...
                for (const enemy of enemies.enemies) {
                    if (!enemy.alive)
                        continue;
                    targets.push(enemy.weakPoint.visible
                        ? enemy.weakPointWorld(new THREE.Vector3())
                        : enemy.centerWorld(new THREE.Vector3()));
                }
                return selectAimAssistDirection(origin, direction, targets, range, maximumAngle, (target) => this.map.collision.segmentBlocked(origin.x, origin.z, target.x, target.z));
            });
            ...
            this.hud.show(true);
            this.mobile.show(false);
            if (!this.save.value.tutorialComplete)
    ...
    wireRun(run) {
        ...
        run.projectiles.onPlayerHit = (damage, source) => this.damagePlayer(damage, source);
        ...
        run.waves.onWaveStarted = (definition) => {
            ...
            this.effects.announce(definition.boss ? `BOSS WAVE ${definition.wave}` : `WAVE ${definition.wave}`, definition.boss ? 'danger' : 'normal');
    ...
    startWave() {
        const run = this.run;
        if (!run || run.state !== 'between')
            return;
        if (run.waves.wave === 0 || !run.waves.active)
            run.waves.startNext();
        this.input.resetAll();
        run.state = 'playing';
        this.hud.show(true);
    ...
    update(delta, now) {
        ...
        if (input.selectWeapon !== undefined) {
            run.weapons.switchTo(['pistol', 'smg', 'shotgun'][input.selectWeapon]);
        }
        if (input.switchDelta !== 0)
            run.weapons.cycle(input.switchDelta);
        run.player.update(delta, input);
        run.weapons.update(delta, now, input.fire, input.reload, input.aim, input.lookSource);
    ...
    updateMenu(delta) {
        ...
        if (glow)
            glow.scale.y = 1 + Math.sin(this.menuTime * 10) * 0.05;
    }
    handleShot(event) {
        const run = this.run;
        if (!run)
            return;
        const permanentLevel = event.weapon === 'pistol'
        ...
        for (const hit of hits) {
            ...
            if (hit.critical)
                run.stats.criticalHits += 1;
            if (this.save.value.settings.hitMarkers)
                this.effects.hitMarker(hit.critical);
            ...
            if (this.save.value.settings.damageNumbers) {
                ...
                this.damageNumbers.show(hit.damage, hit.critical, (projected.x * 0.5 + 0.5) * innerWidth, (-projected.y * 0.5 + 0.5) * innerHeight);
            }
            const lifeSteal = run.upgrades.level('lifesteal');
            if (lifeSteal > 0) {
                const request = cappedHealingRequest(hit.damage * 0.02 * lifeSteal, run.lifeStealHealedThisWave, lifeStealCap(lifeSteal));
                if (request.allowed > 0) {
                    const healed = run.playerStats.heal(request.allowed);
                    run.lifeStealHealedThisWave += healed;
                }
            }
            if (run.upgrades.level('ricochet') > 0 &&
            ...
            if (hit.killed &&
                ...
                run.enemies.damageRadius(hit.enemy.group.position, 3.5, 45 * run.upgrades.level('explosion'));
            }
        }
    }
    damagePlayer(amount, source) {
        ...
        if (this.save.value.settings.vibration && navigator.vibrate)
    ...
    enemyKilled(kind) {
        ...
        if (kind === 'runner')
            run.stats.runnersDefeated += 1;
        else if (kind === 'brute')
            run.stats.brutesDefeated += 1;
        else if (kind === 'spitter')
            run.stats.spittersDefeated += 1;
        else
    ...
    collectPickup(kind, value) {
        const run = this.run;
        if (!run)
            return;
        this.audio.play('pickup');
        this.particles.burst(run.player.root.position.clone().add(new THREE.Vector3(0, 1, 0)), kind === 'health' ? PALETTE.green : kind === 'armor' ? PALETTE.blue : PALETTE.orange, 5, 2);
        if (kind === 'coin') {
            run.stats.coins += Math.round(value * run.upgrades.multiplier('coins', 0.18));
            run.stats.coinsCollected = run.stats.coins;
        }
        else if (kind === 'health')
            run.playerStats.heal(value);
        else if (kind === 'armor')
            run.playerStats.addArmor(value);
        else if (kind === 'smgAmmo')
            run.weapons.weapons.smg.reserve += value;
    ...
    resume() {
        const run = this.run;
        if (!run || run.state !== 'paused' || run.deathDialog)
            return;
        this.input.resetAll();
        run.state = 'playing';
        this.ui.closePause();
    ...
    handleDeath() {
        const run = this.run;
        if (!run || run.deathDialog || run.state === 'ending')
            return;
        run.state = 'paused';
        this.input.resetAll();
        this.mobile.show(false);
    ...
    revive() {
        const run = this.run;
        if (!run || run.playerStats.revived || run.stats.coins < 80)
            return;
        run.stats.coins -= 80;
    ...
    finishRun(victory) {
        ...
        run.stats.wave = run.waves.wave;
        const credits = Math.round(calculateRunCredits(run.stats) *
        ...
        this.ui.showResults(run.stats, credits, this.ads.available('doubleCredits'));
    }
    commitRun(run, credits) {
        this.save.mutate((data) => {
            ...
            if (run.stats.victory) {
                stats.wins += 1;
                if (run.difficulty === 'soldier')
                    stats.soldierWins += 1;
                if (run.difficulty === 'veteran')
                    stats.veteranWins += 1;
            }
    ...
    applySettings(settings) {
        ...
        this.hud.setSettings(settings.uiScale, settings.crosshairSize, settings.crosshairOpacity, settings.highContrast);
...
function createRunStats(difficulty) {
    return {
        totalRuns: 1,
        wins: 0,
        highestWave: 0,
