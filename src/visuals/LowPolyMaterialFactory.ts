...
export class LowPolyMaterialFactory {
  ...
  material(color: number, emissive = 0x000000, emissiveIntensity = 0): any {
  ...
  transparent(color: number, opacity: number): any {
  ...
  dispose(): void { this.cache.forEach((material) => material.dispose()); this.cache.clear(); }
}
