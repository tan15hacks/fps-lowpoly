...
export class Enemy {
    ...
    armorActive = false;
    flashTimer;
    constructor(kind, wave, difficulty, materials) {
        this.kind = kind;
        ...
        this.body = new THREE.Mesh(new THREE.DodecahedronGeometry(width * 0.62, 0), materials.material(color));
        this.body.scale.set(1, height / (width * 1.2), 0.8);
        this.body.position.y = height * 0.48;
        this.body.castShadow = true;
        this.group.add(this.body);
        this.head = new THREE.Mesh(new THREE.IcosahedronGeometry(width * 0.42, 0), materials.material(color));
        this.head.position.y = height * 0.88;
        this.head.position.z = -width * 0.22;
        this.head.castShadow = true;
        this.group.add(this.head);
        this.weakPoint = new THREE.Mesh(new THREE.OctahedronGeometry(width * 0.16, 0), materials.material(PALETTE.orange, PALETTE.orange, 1.5));
        ...
        for (let index = 0; index < legs; index += 1) {
            const leg = new THREE.Mesh(new THREE.CylinderGeometry(width * 0.11, width * 0.14, height * 0.45, 5), materials.material(color));
            leg.position.set((index % 2 ? 1 : -1) * width * 0.24, height * 0.18, (index >= 2 ? 1 : -1) * width * 0.2);
            leg.rotation.z = (index % 2 ? 1 : -1) * 0.15;
            this.group.add(leg);
        }
        if (kind === 'boss' || kind === 'brute') {
            for (const side of [-1, 1]) {
                const arm = new THREE.Mesh(new THREE.CylinderGeometry(width * 0.13, width * 0.2, height * 0.58, 6), materials.material(color));
