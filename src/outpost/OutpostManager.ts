...
export class OutpostManager {
  ...
  private scanTimer = 0;
  private healingCharge = HEALING_STATION.chargePerWave;
  private fenceCooldown = new WeakMap<object, number>();
  private readonly timers = new Set<number>();
  private disposed = false;
  onTurretDamage?: (damage: number) => void;

  constructor(
    private systems: MapSystems,
  ...
  update(delta: number, playerPosition: any): void {
    ...
    if (
      ...
      const amount = Math.min(
        this.healingCharge,
        HEALING_STATION.combatRate * delta * this.upgrades.multiplier('healing', 0.25),
      );
      this.healingCharge -= this.player.heal(amount);
    }
    if (this.power.isActive('scanner') && this.scanTimer <= 0) {
      for (const enemy of this.enemies.enemies) {
        if (!enemy.alive || enemy.group.position.distanceTo(playerPosition) >= 18) continue;
        enemy.weakPoint.scale.setScalar(1.45);
        this.schedule(() => {
          if (enemy.alive) enemy.weakPoint?.scale.setScalar(1);
        }, 800);
      }
      this.scanTimer = 4;
    }
  ...
  private updateFence(delta: number): void {
    for (const enemy of this.enemies.enemies) {
      ...
      const near =
        Math.abs(Math.abs(position.x) - ELECTRIC_FENCE.boundary) < 0.8 ||
      ...
      this.schedule(() => {
        if (enemy.alive) enemy.speed /= ELECTRIC_FENCE.slow;
      }, 500);
