...
function createManager(quickSlingStacks = 0) {
    ...
    for (let index = 0; index < quickSlingStacks; index += 1)
        upgrades.apply('switch');
    const stats = new PlayerStats(permanent, upgrades);
    const materials = new LowPolyMaterialFactory();
    const audio = { play: () => undefined };
    const manager = new WeaponManager(camera, new PlayerCamera(camera, { ...DEFAULT_SETTINGS }), stats, upgrades, audio, { ...DEFAULT_SETTINGS }, materials);
