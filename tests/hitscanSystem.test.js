...
describe('hitscan system', () => {
    it('keeps every pellet inside a circular spread radius', () => {
        ...
        let index = 0;
        const rays = createHitscanRays(new THREE.Vector3(), new THREE.Vector3(0, 0, -1), new THREE.Vector3(1, 0, 0), new THREE.Vector3(0, 1, 0), 4, 0.1, 50, () => values[index++] ?? 0);
    ...
    it('returns the nearest forward sphere intersection', () => {
        const distance = raySphereDistance(new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 0, -1), new THREE.Vector3(0, 0, -10), 2);
        expect(distance).toBeCloseTo(8, 6);
    });
    it('reports an immediate hit when the ray starts inside a sphere', () => {
        const distance = raySphereDistance(new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 0, -1), new THREE.Vector3(0, 0, 0), 2);
